...
public:
    ...
    void start() override {
        ...
        constexpr size_t handle_size =
            ccl_device::device_ipc_memory_handle::get_size_for_serialize();
        ...
        if (event.test()) {
            ...
            */
            ...
            while (recv_data_size > 0) {
                ...
                LOG_DEBUG(
                    "Received IPC rank: ", received_rank_idx, ", on rank: ", comm_addr.to_string());
                ...
                while (num_handles < send_handles.size()) {
                    ...
                    std::shared_ptr<ccl_ipc_gpu_comm> ipc_mem_owner;
                    {
                        ...
                        auto& ipc_device_cont =
                            ccl_tuple_get<indexed_device_container<ccl_ipc_gpu_comm>>(device_cont);
                        ...
                        if (ipc_device_cont_it == ipc_device_cont.end()) {
                            if (received_rank_idx != comm_addr.rank) {
                                LOG_ERROR("No device owner for ipc handle detected, ipc handle: ",
                                          native::to_string(recv_ip_handle->get()),
                                          ", suggested device handle: ",
                                          *recv_ip_handle->get_owner().lock(),
                                          ", suggested rank: ",
                                          received_rank_idx,
                    ...
                    LOG_DEBUG("Find gpu device: ",
                              ipc_mem_owner->to_string(),
                              ", IPC handle: ",
                              native::to_string(recv_ip_handle->get()));
            ...
            LOG_INFO("All handles deserialized. Start ipc kernel arguments binding",
                     ", rank: ",
                     comm_addr.to_string());
            for (auto& dev_handle_pair : foreign_device_ipc_mem_storage) {
                ...
                LOG_INFO(
                    "Bind kernel arguments: ", handles.size(), ", rank: ", comm_addr.to_string());
                CCL_ASSERT(handles.size() == send_handles.size(),
                           "Received unexpected memory handles count");
                ...
                typename kernel_ipc_typed::tmp_recv_buf_arg_type tmp_recv_buf =
                    reinterpret_cast<typename kernel_ipc_typed::tmp_recv_buf_arg_type>(
                        handles.at(0).get().pointer);
                unreach_rank_main_func
                    .template set_arg<typename kernel_ipc_typed::tmp_recv_buf_arg>(tmp_recv_buf);

                typename kernel_ipc_typed::income_data_flag_arg_type inc =
                    reinterpret_cast<typename kernel_ipc_typed::income_data_flag_arg_type>(
                        handles.at(1).get().pointer);
                unreach_rank_main_func
                    .template set_arg<typename kernel_ipc_typed::income_data_flag_arg>(inc);

                typename kernel_ipc_typed::ready_to_recv_flag_arg_type ready =
                    reinterpret_cast<typename kernel_ipc_typed::ready_to_recv_flag_arg_type>(
                        handles.at(2).get().pointer);
                unreach_rank_main_func
                    .template set_arg<typename kernel_ipc_typed::ready_to_recv_flag_arg>(ready);
            }
